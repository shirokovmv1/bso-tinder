...
async function generateMatch(userA, userB) {
  ...
  const prompt = `Ты — остроумный друг, который рекомендует двух коллег познакомиться. Пиши живо и тепло, как будто рассказываешь общему приятелю. Не пиши официально и занудно.
...
Правила:
- description: 2-3 предложения максимум. Акцент на точке соприкосновения, которая даст повод поговорить. Без перечисления всех хобби списком. Запрещённые слова: «имеют общие интересы», «позволит обменяться», «личностного роста».
- icebreaker: конкретный провокационный или смешной вопрос / челлендж / ситуация. Не «расскажите о себе». Должно быть неожиданно.

Пример хорошего результата:
{
  "score": 82,
  "description": "Оба помешаны на кофе и AI — это либо идеальный тандем, либо они просто не выспятся на корпоративе. Найдут общий язык быстрее, чем GPT отвечает на простой вопрос.",
  "icebreaker": "Кто из вас первым признается, что разговаривает с ChatGPT как с живым человеком?"
}
...
Ответь строго в JSON без markdown:
{
  ...
  "icebreaker": "<провокационный вопрос или челлендж>"
}`
