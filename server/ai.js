...
async function generateMatch(userA, userB) {
  ...
  const prompt = `Ты помогаешь двум сотрудникам одной компании познакомиться на корпоративном мероприятии.
Контекст: это внутренний корпоратив строительно-логистической компании «БСО Логистик». Все уже пришли — их позвала компания. Задача — найти интересных коллег внутри, а не искать работу или продавать себя. Пиши как умный, остроумный коллега, который знает обоих и говорит третьему: «слушай, вы точно сойдётесь».
...
1. description — 2-3 живых предложения. Найди одну настоящую точку соприкосновения и сделай её интересной. Никаких списков хобби через запятую.
2. Запрещены слова и конструкции: «имеют общие интересы», «позволит обменяться», «личностного роста», «эффективного взаимодействия», «синергия», «нетворкинг».
3. Запрещено придумывать новые глаголы из существительных (не «логистировать», не «фотографировать бизнес», не «дизайнить логистику»). Если не знаешь как описать — используй простые слова.
4. icebreaker — один вопрос или мини-челлендж, адресованный одному из них (не обоим сразу). Провокационный, смешной или неожиданный. Не «расскажите о себе».
5. Не упоминай компанию по названию, не пиши «на корпоративе» прямым текстом — читатель и так знает где находится.

ПРИМЕР ХОРОШЕГО РЕЗУЛЬТАТА:
{
  "score": 79,
  "description": "Оба 4 года в компании и оба помешаны на кофе — это либо взаимное уважение, либо конкуренция за последний стакан на шведском столе. А разница в работе (логистика vs фото) — лучший повод обменяться историями про хаос, который называют рабочим процессом.",
  "icebreaker": "Алексей, если бы тебе дали в подчинение фотографа на один день — что бы ты заставил его снять первым делом?"
}

Ответь строго в JSON без markdown:
{
  "score": <число 0-100>,
  "description": "<живой питч 2-3 предложения>",
